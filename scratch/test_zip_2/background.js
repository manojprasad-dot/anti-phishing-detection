...
chrome.tabs.onUpdated.addListener(async (tabId, changeInfo, tab) => {
  if (changeInfo.status === "loading") {
    // Page is reloading or navigating away, clear the scan cache for this tab
  ...
  if (isEmailAppUrl(url)) return;

  // Trigger scan on "complete" OR when URL changes (for SPAs)
  if (changeInfo.status === "complete" || (changeInfo.url && changeInfo.url !== scannedTabs.get(tabId))) {
    // Skip if we already scanned this exact URL in this tab
    if (scannedTabs.get(tabId) === url) return;
    scannedTabs.set(tabId, url);

    console.log(`[PhishGuard] Page loaded/navigated — scanning: ${url}`);
    
    // Ensure content script shows scanning badge (useful for SPA navigation)
    chrome.tabs.sendMessage(tabId, { type: "SHOW_SCANNING" }).catch(() => {});
...
function isEmailAppUrl(url) {
  try {
    ...
    return EMAIL_HOST_PATTERNS.some((host) => hostname === host);
  } catch {
    return false;
  }
}
